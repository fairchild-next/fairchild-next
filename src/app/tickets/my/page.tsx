        ...
        <div className="space-y-6">
          ...
          {fetching ? (
            <div className="space-y-4">
              {[1, 2].map((i) => (
                <div key={i} className="rounded-xl border border-[var(--surface-border)] overflow-hidden animate-pulse">
                  <div className="aspect-[2/1] bg-[var(--surface-border)]" />
                  <div className="p-4 space-y-3">
                    <div className="h-4 w-2/3 rounded bg-[var(--surface-border)]" />
                    <div className="h-3 w-1/3 rounded bg-[var(--surface-border)]" />
                    <div className="h-36 w-36 rounded bg-[var(--surface-border)]" />
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="flex flex-col items-center gap-4 py-10 text-center">
              <div className="w-14 h-14 rounded-full bg-[var(--surface)] border border-[var(--surface-border)] flex items-center justify-center">
                <svg className="w-7 h-7 text-[var(--text-muted)]" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden>
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 110 4v3a2 2 0 002 2h14a2 2 0 002-2v-3a2 2 0 110-4V7a2 2 0 00-2-2H5z" />
                </svg>
              </div>
              <div>
                <p className="font-medium text-[var(--text-primary)]">No tickets yet</p>
                <p className="text-sm text-[var(--text-muted)] mt-1">Purchase admission or event tickets to see them here.</p>
              </div>
              <Link
                href="/tickets"
                className="px-5 py-2.5 rounded-xl bg-[var(--primary)] text-white text-sm font-semibold transition hover:opacity-90 active:opacity-80"
              >
                Browse Tickets →
              </Link>
            </div>
...
function pastGroupKey(group: Ticket[]): string {
  const first = group[0];
  if (!first) return "unknown";
  return first.event_id
    ? `${first.order_id ?? "?"}_event_${first.event_id}`
    : `${first.order_id ?? "?"}_${first.slot_id ?? "flex"}`;
}
